...
export const PublishModal: React.FC<PublishModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in">
      <div className="bg-[#1E293B] border border-slate-700 rounded-3xl w-full max-w-xl overflow-hidden shadow-2xl text-cms-small">
        ...
        <div className="bg-gradient-to-r from-slate-900 to-[#1E293B] p-6 border-b border-slate-700 relative">
          ...
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-amber-500/20 text-amber-300 font-bold uppercase tracking-wider text-cms-small mb-2 border border-amber-500/30">
        ...
        <div className="p-6 space-y-6">
          {publishSuccess ? (
            <div className="py-8 text-center space-y-3">
              ...
              <p className="text-cms-small text-slate-300 max-w-sm mx-auto">
            ...
            <>
              ...
              <div className="p-4 rounded-2xl bg-slate-900 border border-slate-800 space-y-3">
                ...
                {errors.length > 0 && (
                  <div className="p-3 rounded-xl bg-rose-500/10 border border-rose-500/30 space-y-1">
                    ...
                    {errors.map(err => (
                      <div key={err.id} className="text-cms-small text-rose-300">
                        • {err.message}
                      </div>
                    ))}
                  </div>
                ...
                {warnings.length > 0 && (
                  <div className="p-3 rounded-xl bg-amber-500/10 border border-amber-500/30 space-y-1">
                    ...
                    {warnings.slice(0, 3).map(warn => (
                      <div key={warn.id} className="text-cms-small text-slate-300">
                        • {warn.message}
                      </div>
                    ))}
                  </div>
                ...
                {errors.length === 0 && warnings.length === 0 && (
                  <div className="text-cms-small text-emerald-400 font-semibold flex items-center gap-1.5">
              ...
              <div className="grid grid-cols-3 gap-3 text-center">
                <div className="p-3 rounded-xl bg-slate-900 border border-slate-800">
                  <div className="text-slate-400 text-cms-small uppercase font-bold">Tea Stories</div>
                  <div className="text-cms-card text-white mt-0.5">
                    {(draftState.teaStories || []).filter(p => p.isVisible).length}
                  </div>
                </div>
                <div className="p-3 rounded-xl bg-slate-900 border border-slate-800">
                  <div className="text-slate-400 text-cms-small uppercase font-bold">Media Slots</div>
                  <div className="text-cms-card text-white mt-0.5">
                    {Object.keys(draftState.mediaSlots).length}
                  </div>
                </div>
                <div className="p-3 rounded-xl bg-slate-900 border border-slate-800">
                  <div className="text-slate-400 text-cms-small uppercase font-bold">Menu Links</div>
                  <div className="text-cms-card text-white mt-0.5">
              ...
              <div className="flex items-center justify-between pt-4 border-t border-slate-700">
                <button
                  ...
                  className="px-4 py-2.5 rounded-xl bg-slate-800 hover:bg-rose-500/20 text-slate-400 hover:text-rose-300 text-cms-btn flex items-center gap-1.5 disabled:opacity-40"
                >
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    className="px-6 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-bold uppercase tracking-wider text-cms-small flex items-center gap-2 shadow-lg disabled:opacity-40"
                  >
